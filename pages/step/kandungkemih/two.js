...
import { kandungkemih_risk_id, kandungkemih_risk_en, kandungkemih_risk2_id, kandungkemih_risk2_en } from "@/constant/kandungkemih"
...
export default function Two() {
    ...
    const onSubmitForm = async (values) => {
        ...
        context.setqr(data)
        context.seturls('kandungkemih')
        router.push('/step/data_diri')
    }

    const risk = i18n.language == 'id' ? kandungkemih_risk_id : kandungkemih_risk_en;
    const risk2 = i18n.language == 'id' ? kandungkemih_risk2_id : kandungkemih_risk2_en;

    return (
        <Layout title={t("step-2-title")} back={'/step/kandungkemih/one'}>
            <div className="flex flex-wrap justify-between items-center px-5 pt-5 bg-white overflow-hidden w-full">
                <Formik
                    initialValues={{ qr1: '', qr2: '', qr3: '', q3_1: '' }}
                    ...
                    {({ errors, touched }) => (
                        <Form className="flex flex-col justify-between min-h-screen pb-6 w-full">
                            <div className='flex flex-col mb-32'>
                                ...
                                <div className='py-2'>
                                    <p className='font-bold text-gray-700 text-sm'>{t("risk-desc", { ns: 'kandungkemih' })}</p>
                                </div>
                                <hr className=' decoration-dashed mb-3' />
                                <div className='mb-4'>
                                    <label className="block mb-1 font-bold text-gray-700 text-sm">{t("qr1", { ns: 'kandungkemih' })}</label>
                                ...
                                <div className='mb-4'>
                                    <label className="block mb-1 font-bold text-gray-700 text-sm">{t("qr2", { ns: 'kandungkemih' })}</label>
                                    <ul className="list-decimal">
                                        {
                                    ...
                                    <button type="button" onClick={() => setShowModal2(true)} className="text-warning-500 underline">{t("step-2-title-3-2")}</button>
                                ...
                                <div className='mb-4'>
                                    <label className="block mb-1 font-bold text-gray-700 text-sm">{t("qr3", { ns: 'kandungkemih' })}</label>
                                    <div className='flex flex-row form-check'>
                                        <label className='text-sm align-middle form-check-label inline-block'>
                                    ...
                                    <div className="mt-3">
                                        <Field name="qr3_1" className="w-full py-2 px-3 rounded-lg text-sm text-gray-700 bg-white border border-gray-200 overflow-hidden focus:outline-none focus:border-primary-700 focus:ring-primary-700 focus:ring-1" type="text" placeholder={t("qr3-2", { ns: 'kandungkemih' })} />
                                    </div>
                                </div>
                    ...
                    <div id="defaultModal" tabIndex="-1" className="mx-auto max-w-layout justify-center items-center flex overflow-x-hidden overflow-y-auto inset-0 z-50 outline-none focus:outline-none fixed z-50 md:inset-0 h-modal md:h-full">
                        <div className="relative p-4 w-full h-4/5">
                            <div className="relative bg-white rounded-lg shadow ">
                                ...
                                <div className="flex items-center justify-end p-3 border-t border-solid border-blueGray-200 rounded-b">
                                    <button data-modal-toggle="defaultModal" onClick={() => setShowModal2(false)} type="button" className="bg-secondary-500 text-white font-bold block w-full text-center text-sm p-3 rounded-full">Close</button>
                                </div>
                            </div>
                        </div>
                    </div>
