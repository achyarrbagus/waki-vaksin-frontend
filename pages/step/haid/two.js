...
import { haid_risk_id, haid_risk_en } from "@/constant/haid"
...
export default function Two() {
    ...
    const onSubmitForm = async (values) => {
        ...
        context.setqr(data)
        context.seturls('haid')
        router.push('/step/data_diri')
    }

    const risk = i18n.language == 'id' ? haid_risk_id : haid_risk_en;

    return (
        <Layout title={t("step-2-title")} back={'/step/haid/one'}>
            <div className="flex flex-wrap justify-between items-center px-5 pt-5 bg-white overflow-hidden w-full">
                <Formik
                    initialValues={{ qr1: '', qr2: '', qr3: '' }}
                    ...
                    {({ errors, touched }) => (
                        <Form className="flex flex-col justify-between min-h-screen pb-6 w-full">
                            <div className='flex flex-col mb-32'>
                                ...
                                <div className='py-2'>
                                    <p className='font-bold text-gray-700 text-sm'>{t("risk-desc", { ns: 'haid' })}</p>
                                </div>
                                <hr className=' decoration-dashed mb-3' />
                                <div className='mb-4'>
                                    <label className="block mb-1 font-bold text-gray-700 text-sm">{t("qr1", { ns: 'haid' })}</label>
                                ...
                                <div className='mb-4'>
                                    <label className="block mb-1 font-bold text-gray-700 text-sm">{t("qr2", { ns: 'haid' })}</label>
                                    <div className='flex flex-row form-check mb-3'>
                                        <label className='text-sm align-middle form-check-label inline-block'>
                                ...
                                <div className='mb-4'>
                                    <label className="block mb-1 font-bold text-gray-700 text-sm">{t("qr3", { ns: 'haid' })}</label>
                                    ...
                                    <button type="button" onClick={() => setShowModal(true)} className="text-warning-500 underline">{t("qr3", { ns: 'haid' })}</button>
