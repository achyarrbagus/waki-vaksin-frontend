...
import { vaksin_id, vaksin_en, vaksin } from "@/constant/vaksin"
...
export async function getStaticProps({ locale }) {
    return {
        props: {
            ...(await serverSideTranslations(locale, ["common", "vaksin"]))
        }
    };
}
...
export default function One() {
    ...
    const { t, i18n } = useTranslation(["common", "vaksin"]);
    ...
    const onSubmitForm = async (values) => {
        ...
        context.setqf(data)
        context.setDays(values.q6)
        router.push('/step/vaksin/two')
    }

    const symptoms = i18n.language == 'id' ? vaksin_id : vaksin_en;
    ...
    return (
        <Layout title={t("step-1-title-1")} back={'/surat'}>
            <div className="flex flex-row justify-between items-center px-5 pt-5 bg-white overflow-hidden w-full" >
                <Formik
                    initialValues={{ q1: '', q2: '', q3: '', q4: '', q5: '', q6: '' }}
                        ...
                        <Form className="flex flex-col justify-between h-full w-full pb-6">
                            <div className='flex flex-col mb-32'>
                                <div className='py-4 my-5'>
                                    <h1 className='text-left text-3xl text-gray-700 font-black'>{t("title", { ns: 'vaksin' })}</h1>
                                    <p className='text-md text-gray-600 text-sm font-normal'>{t("step-1-title-3")}</p>
                                </div>
                                <div className=' mb-4'>
                                    <label className="block mb-1 font-bold text-gray-700 text-sm">{t("q1", { ns: 'vaksin' })}</label>
                                    <Field
                                ...
                                <div className=' mb-4'>
                                    <label className="block mb-1 font-bold text-gray-700 text-sm">{t("q2", { ns: 'vaksin' })}</label>
                                    <Field
                                        className="custom-select"
                                        name="q2"
                                        options={vaksin}
                                ...
                                <div className=' mb-4 w-full'>
                                    <label className="block mb-1 font-bold text-gray-700 text-sm">{t("q3", { ns: 'vaksin' })}</label>
                                    <div className="">
                                        <DatePicker
                                            disabledDate={d => !d || d.isAfter(dateNow) || d.isSameOrBefore(moment(date))}
                                            format="YYYY-MM-DD"
                                            defaultPickerValue={moment(dateNow)}
                                            placeholder={t("select_date")}
                                            name="q3"
                                            onChange={(date, dateString) => setFieldValue('q3', dateString)}
                                ...
                                <div className=' mb-4'>
                                    <label className="block mb-1 font-bold text-gray-700 text-sm">{t("q4", { ns: 'vaksin' })}</label>
                                ...
                                <div className=' mb-4'>
                                    <label className="block mb-1 font-bold text-gray-700 text-sm">{t("q5", { ns: 'vaksin' })}</label>
                                    <div className="">
                                        <Field name="q5" className="w-full py-2 px-3 rounded-lg text-sm text-gray-700 bg-white border border-gray-200 overflow-hidden focus:outline-none focus:border-primary-700 focus:ring-primary-700 focus:ring-1" type="text" placeholder={t("more")} />
                                    </div>
                                    {errors.q5 && touched.q5 && <div className='text-red-500 font-light text-sm ml-2'>{errors.q5}</div>}
                                </div>

                                <div className=' mb-4'>
                                    <label className="block mb-1 font-bold text-gray-700 text-sm">{t("how-many-days")}</label>
