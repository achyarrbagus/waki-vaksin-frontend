    ...
    <Layout title="Article" back={`/`}>
      <div className="py-10 px-4 bg-white">
        ...
        <div className="flex justify-between items-center mb-3">
          <div className="text-blue-400 text-md">{article.category || <Skeleton />}</div>
          <div className="text-gray-400 text-md">
            {article.CreatedAt}
          </div>
        </div>
        <h3 className="font-bold text-lg mb-3">{article.title || <Skeleton />}</h3>
        <div className="text-md text-gray-600 font-normal text-justify">
          {article.content ? <div className='article' dangerouslySetInnerHTML={{ __html: article != null && article.content }}></div> : <Skeleton count={15} />}
        </div>
      </div>
    </Layout>
  )
